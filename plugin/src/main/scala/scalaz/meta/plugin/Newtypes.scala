...
//      println(s"annotation.args = ${annotation.args}")
    ...
    def runAnnotation(annotationName: String, subtype: Boolean, clsDef: ClassDef, modDef: Option[ModuleDef]): List[Tree] =
      modDef match {
    ...
    def generateNewType(clsDef: ClassDef,
      ...
      val opsDef = maybeGenerateOpsDef(clsDef, valDef, tparamsNoVar, tparamNames, isDefinedInObject, optimizeOps)
      val companionExtraDefs = opsDef.map(opsDef =>
...
//        generateDerivingMethods(tparamsNoVar, tparamNames, tparamsWild, new FreshNameCreator())
    ...
    def mkBaseTypeDef(clsDef: ClassDef, reprType: Tree, subtype: Boolean): Tree = {
      ...
      (clsDef.tparams, subtype) match {
        case (_, false)      =>  q"type Base             = ${definitions.AnyTpe} { type $refinementName } "
        case (Nil, true)     =>  q"type Base             = $reprType"
        case (tparams, true) =>  q"type Base[..$tparams] = $reprType"
      }
    }

    def mkTypeTypeDef(clsDef: ClassDef,
                      tparamsNames: List[TypeName],
                      subtype: Boolean): Tree =
      (clsDef.tparams, subtype) match {
    ...
    def getInstanceMethods(clsDef: ClassDef,
      ...
      val res = clsDef.impl.body.traverse[Validated, List[DefDef]] {
        case vd: ValDef =>
          if (vd.mods.hasFlag(Flag.CASEACCESSOR) || vd.mods.hasFlag(Flag.PARAMACCESSOR))
            ok(Nil)
